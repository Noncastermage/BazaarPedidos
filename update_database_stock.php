...
try {
    echo "<h1>Actualizando la Base de Datos</h1>";
    ...
    // Verificar si la columna is_delivered existe en la tabla order_items
    ...
    if (!$is_delivered_column_exists) {
        // Añadir la columna is_delivered a la tabla order_items
        $conn->exec("ALTER TABLE order_items ADD COLUMN is_delivered TINYINT(1) DEFAULT 0");
        echo "La columna 'is_delivered' ha sido agregada a la tabla 'order_items'.<br>";
        $_SESSION['success'] .= " La columna 'is_delivered' ha sido agregada a la tabla 'order_items'.";
    } else {
        echo "La columna 'is_delivered' ya existe en la tabla 'order_items'.<br>";
    }
    ...
    echo "<p><strong>Por favor, recarga la página principal para ver los cambios.</strong></p>";
